...
def main():
    ...
    cloud_dir = Path(cfg["features_build"]["cloud_tif_dir"])
    out_csv = Path(cfg["features"]["cloud_csv"])
    ...
    records = []
    for tif in sorted(cloud_dir.glob("*.tif")):
    ...
    out["cloud_mean"] = pivot.mean(axis=1, skipna=True).values
    for m in range(1, 13):
        out[f"cloud_m{m:02d}"] = pivot[m].values
